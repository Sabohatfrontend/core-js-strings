...
function findLongestWord(sentence) {
  const word = sentence.split(' ').sort((a, b) => b.length - a.length)[0];
  return word;
}
...
function getStringFromTemplate(firstName, lastName) {
  return 'Hello, '.concat([firstName, lastName].join(' ')).concat('!');
}
...
function encodeToRot13(str) {
  ...
  const newStr = str
    .split('')
    .map((a) =>
      originalAlpha.indexOf(a) === -1 ? a : cipher[originalAlpha.indexOf(a)]
    )
    .join('');
  return newStr;
}
